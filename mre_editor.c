...
//VMINT ucs2_len_words = 0;   /* used length in words (VMWCHAR) */
...
//static VMINT append_ucs2_words(const VMWCHAR *src, VMINT src_words);
static VMINT append_ucs2_words(VMWCHAR *src, VMINT src_words);
static void ucs2_buf_init_empty(void);
//static VMINT load_utf8_file_to_ucs2_chunked(const VMWCHAR *file_path);
static VMINT load_utf8_file_to_ucs2_chunked(VMWCHAR *file_path);
//static VMINT save_ucs2_buffer_to_utf8_chunked(const VMWCHAR *outfile_path);
static VMINT save_ucs2_buffer_to_utf8_chunked(VMWCHAR *outfile_path);
...
//VMINT app_load_file_into_editor(const VMWCHAR *inpath);
VMINT app_load_file_into_editor(VMWCHAR *inpath);
//VMINT app_save_editor_to_file(const VMWCHAR *outpath);
VMINT app_save_editor_to_file(VMWCHAR *outpath);
...
static VMINT load_utf8_file_to_ucs2_chunked(VMWCHAR *file_path)
{
    ...
    VMFILE fh = vm_file_open(file_path, MODE_READ, FALSE); //TRUE
    if (fh < 0) return -1;
...
static VMINT save_ucs2_buffer_to_utf8_chunked(VMWCHAR *outfile_path)
{
    ...
    VMFILE fh = vm_file_open(outfile_path, MODE_CREATE_ALWAYS_WRITE, FALSE); //TRUE
    if (fh < 0) return -2;
