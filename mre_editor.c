...
static VMINT job(VMWCHAR *filepath, VMINT wlen) {
    ...
    VMCHAR filename_ascii[260] = {0};
    VMCHAR output_ascii[260] = {0};
    VMCHAR ext_ascii[8] = {0};

    vm_ucs2_to_ascii(filename_ascii, sizeof(filename_ascii), filepath);
    extract_end_text(ext_ascii, filename_ascii, '.');   // ext_ascii = "txt"
    strncpy(output_ascii, filename_ascii, strlen(filename_ascii) - strlen(ext_ascii) - 1);
    output_ascii[strlen(filename_ascii) - strlen(ext_ascii) - 1] = '\0';
    strcat(output_ascii, "_");
    strcat(output_ascii, ext_ascii);
    strcat(output_ascii, ".txt");
    vm_ascii_to_ucs2(outfile, sizeof(outfile), output_ascii);
