...
void handle_sysevt(VMINT message, VMINT param) {
    ...
    case VM_MSG_ACTIVE:
        if(layer < 0) {
            layer = vm_graphic_create_layer(0,0,vm_graphic_get_screen_width(),vm_graphic_get_screen_height(),-1);
        ...
        if(editor <= 0) {
            ...
            /* Force softkey bar to stay visible */
            //vm_editor_set_is_draw_ime_floating_indicator(VM_TRUE);
